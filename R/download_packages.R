...
download_packages <- function(app_dir, pkgs_path, pkgs, repo, remotes, auth_user, auth_token) {
  ...
  if (remotes[1] != "none") {
    ...
    req_deps <- remotes[!downloaded_deps]
    if (length(req_deps) > 0) {
      cat("Downloading remotes... \n - ")
      cat(remotes, sep = "\n - ")

      github_remotes <- lapply(remotes, devtools:::github_remote, username = auth_user,
                        auth_token = auth_token, host = "https://api.github.com")

      zip_files <- lapply(github_remotes, devtools:::remote_download.github_remote)
